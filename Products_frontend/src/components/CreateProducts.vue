<template>
    <div class="container mx-auto mt-8">
      ...
      <form @submit.prevent="submitForm" class="max-w-md mx-auto bg-white p-8 rounded-md shadow-md">
        ...
        <div>
          <button @click="createProduct" type="submit" class="bg-blue-500 text-white p-2 rounded-md hover:bg-blue-600">Save Changes</button>
        </div>
      </form>
    </div>
  </template>
  ...
  async function createProduct() {
    let products = await axios.post(`http://127.0.0.1:8000/api/products/store`, {
      name: productName,
      price: price,
      category: category
    });
